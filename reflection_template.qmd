...
#lab 2 Q1
#this works if I have my directory set to where the csv lives
...
#this also reaches into a directory if I have one already placed somewhere else.
...
teacher_evals_clean <- teacher_evals |>
  ...
  select(course_id, 
         ...
         academic_degree, 
         seniority, 
         sex) #I am selecting all of these columns to keep
...
#alright so what if we just removed the other columns with a -? Ta-dah
...
#filtering for adresses which are labeled as characters
...
#using functions from stringr package I am looking for IDs starting with a character sequence.
...
#I am changing the seniority level to have meaning that makes sense like junior, senior, and very senior
...
#using fct_recode to rename infant, toddler, and preschooler
...
#fct_reorder is being used to reorder these columns
...
#using str_remove I am removing the word county from the observations or data and then it will be reworked to be put in different census regions
...
# #https://chatgpt.com/
# In the **SQL Murder Mystery (“SQL City”)** challenge, an **anti-join** is most useful at the point where you need to find **which suspects *do not* appear somewhere they should** — in other words, when eliminating people based on missing evidence.
# 
# ### ⭐ Best Place to Use an Anti-Join
# 
# The clearest spot is when you’re trying to answer questions like:
# 
# > **“Which people in the `person` table *don’t* appear in the list of valid suspects derived from clues?”**
# 
# More specifically, many solvers use an **anti-join** when narrowing down suspects by excluding anyone who **doesn’t have matching evidence in related tables**, such as:
# 
# * People **not** found in the `income` table when looking for someone wealthy
# * People **not** holding a gym membership
# * People who **didn’t appear** at the crime scene based on `interview` or `facebook_event_checkin` tables
# * People **not** matching physical clues like height, hair color, etc.
# 
# ### 💡 Concrete Example
# 
# Suppose you’ve determined from the clues that suspects must have a gym membership. An anti-join can give you the people who *don’t*:
# 
# ```sql
# SELECT p.*
# FROM person p
...
# ```
# 
# This returns **people who do not have a Get Fit Now membership** — effectively removing people who *can't* be the suspect.
# 
# ### 🧠 Why Anti-Joins Help
# 
# Anti-joins let you turn logic like:
# 
# * “We know the killer **must** have X”
# 
# into:
# 
# * “Remove everyone who **doesn’t** have X.”
# 
...
#I think I needed an account with Chat to just share a link so I included the response when I asked where can I use an anti-join in the SQL Murder Mystery Challenge
...
#going from a wide data set to a long one
...
#going from long to a wide dataset
...
ggplot(data = surveys, 
  ...
  labs( x= "Weight (g)",
        y = NULL, 
        title = "Relationship between weight and hindfoot length across rodent species", 
        subtitle = "Hindfoot Length (mm)") +
  theme(plot.subtitle = element_text(hjust = 0.5))

#making sure to make a new row after a , or a + so I can easily edit the plot if I need to
...
#making sure my pipes are at the end of each row
...
#using the range function instead of min and max over and over again
...
#mhi_2018 and mc_infant are numeric
...
#species is categorical and weight is numeric
...
#I am plotting years of fishing trips on the x-axis
```
...
#in theme I alter the legend position and the scaling.
```
...
#using the labs function with a title and a subtitle
